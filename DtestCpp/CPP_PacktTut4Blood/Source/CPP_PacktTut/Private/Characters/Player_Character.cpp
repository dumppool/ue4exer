...
float APlayer_Character::TakeDamage(float DamageAmount, FDamageEvent const & DamageEvent, AController * EventInstigator, AActor * DamageCauser)
{
	...
	GEngine->AddOnScreenDebugMessage(-1, 0.0f, FColor::Yellow,
		FString::Printf(TEXT("Current Health %.2f "), CurrentHealth));

	UE_LOG(LogTemp, Log, TEXT("TakeDamage Current Health: (%f)  "), CurrentHealth);
...
void APlayer_Character::OnDeath_Implementation()
{
	...
	// Detach the gun mesh because it will be trying to attach to a socket
	// that doesn't exist when we set the mesh to null
